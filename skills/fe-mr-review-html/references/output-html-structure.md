...
對齊 fe-mr-review 的高風險區分流（不是舊的「拉低/拉高」表）。掃過 diff，把觸及身分驗證/審核狀態機/權限/個資/表單完整性的段落列成高風險區表，**人工確認的點寫在「需人工確認」欄**（取代被移除的 Human Review Checklist）；其餘為一般區。
...
<section class="risk-triage">
  ...
  <table class="risk-table">
    ...
    <tbody>
      <tr><td><code>{檔案:行數}</code></td><td>{身分驗證/審核狀態機/權限/個資/表單完整性}</td><td>{後果}</td><td>{確認什麼業務規則 / 跨團隊契約}</td></tr>
    </tbody>
  </table>

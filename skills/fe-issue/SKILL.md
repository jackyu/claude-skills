---
name: fe-issue
description: 將 PM 需求 Issue 轉換為前端技術 Issue 草稿。使用時機：(1) 用戶要求將 PM issue 轉為 RD/前端 issue, (2) 用戶提到需求拆解、技術拆解、issue 拆解, (3) 用戶要求分析 PM 需求並產出開發任務, (4) 用戶提到開發任務規劃, (5) 用戶貼上 PM issue 內容要求轉成技術 issue。即使用戶只是提到「幫我看一下這個需求怎麼拆」也應觸發此 skill。
---

# PM to Frontend Issue

將 PM/PD 的需求 Issue 轉化為前端技術 Issue 草稿，包含技術描述（或 `/spec` 產出的規格書）、驗收條件、API 依賴、實作範圍。
...
**規格主體（規格書 或 功能描述，擇一）**
- 由 `/spec` 流程進來、已附規格書 → 規格書全文放進「## 規格書」作為規格主體，不另寫功能描述
- 單獨使用 fe-issue → 寫「功能描述」：用前端技術語言重新描述需求，不要複製 PM 原文；有結構性變更用樹狀圖呈現（component tree／file tree 的選用與畫法依 `fe-show-me`）；標注平台限制
...
- **Micro（< 1 天）**：使用「極簡三段」模式 — 關聯 + 實作範圍 + 預估一行。
  - 實作範圍用白話、動詞開頭（如「將 X 改為 Y」「替換 Z 圖片」），盡可能附上檔案路徑與行數
  - AC 與功能說明合併進實作範圍內，以一句話帶過
  - **不產出**：規格書、功能描述、驗收條件、API 依賴、技術備註、預估工時（獨立 section）、規格疑問警語

- **標準（≥ 1 天）**：使用完整模版 — 規格主體 + AC + API 依賴 + 扁平編號的「實作範圍」清單 + 預估工時 + 技術備註。

用戶可隨時要求切換層級。

開發任務的細部拆解在開發階段（`/start` 之後）依規格書進行，issue 本身寫到「實作範圍」為止。

**長度判準**

標準層級的長度限制依 `references/frontend-issue-template.md` 頂部的判準，產出後自檢，超標先砍雜訊再輸出。

**檔案路徑（架構規範）**

- 「實作範圍」項目中列出的檔案路徑，必須符合 `fe-arch` skill 的架構規範——先載入 fe-arch，依其決策流程決定位置，不要憑印象寫路徑
- 從 issue 階段就把正確路徑寫進實作範圍，開發者照著建檔，從源頭對齊

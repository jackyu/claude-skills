...
| 步驟 | 做什麼 |
|---|---|
| 1 拿素材 | `git diff origin/main...HEAD` 加未 commit 改動；diff 空就改考指定檔案；超過 500 行請你挑 2–3 個區塊 |
| 2 備題庫 | 一次備 5–7 題，每題先寫好標準答案與常見誤解才問；題型四類：為什麼這樣做、為什麼不那樣做、API 契約、影響範圍 |
| 3 一題一答 | 每題綁 `檔案:行號`，你打字回答，AI 評 ✅／🟡／❌，最多追問一次；不接受「大概知道」 |
| 4 導師模式 | 你說不知道就講解（需要時叫 fe-show-me 畫圖），講完換個角度再問一次同一觀念 |
| 5 掌握度報告 | 懂／半懂／不懂三欄、reviewer 可能追問的三題、補課紀錄；寫進 Heptabase，MCP 不在就落 `.claude/notes/<project_name>/`（依專案名分一層） |

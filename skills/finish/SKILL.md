...
- **會跑**：A1 開 MR、A2／B2 保留、A3 丟棄、B1 清理已合併。丟棄尤其值得回顧——「為什麼這個分支最後被丟掉」通常是強訊號。
- **不跑**：finish 在 Step 1（人在 `main`／`rc/*`、有未 commit 變更）或 Step 2（遠端連不上）中止時。什麼都還沒收尾，就沒有收尾可回顧。

retro **只產出提案**、逐項核可才動手，收尾結果不受它影響。retro 報告沒有訊號、或使用者一項都不勾，都不改變 finish 已完成的狀態。

...
## 3. 高風險域強制規則（依目前產品線校準，非通用券商答案）

以下領域依目前產品線（證券業申請線上前台／審核管理後台）校準，完整定義見 `fe-mr-review/references/high-risk-zones.md`；接手其他情境（如交易、報價）需另外定義。以下領域的 API 回應**必須**在 fetch 邊界 `parse()`，禁止 `as` 型別斷言跳過、禁止 `z.any()`：

| 高風險域 | 範例欄位 | 風險 |
|----------|----------|------|
| 身分驗證 / KYC | 證件辨識結果、比對狀態 | 核身誤判 |
| 審核狀態機 | 送件/補件/覆核/核准/退件狀態 | 狀態誤判導致案件卡住或誤放行 |
| 權限邊界 | 案件歸屬、審核人員權限 | 越權操作、看到不屬於自己的案件 |
| 個資欄位 | 身分證字號、財力證明 | 外洩、日誌洩漏 |
| 表單完整性 | 多步驟表單暫存、草稿版本 | 資料遺漏、版本不一致 |
